...
class Jugador(Thread):
    ...
    def intentar_capturar_bandera(self) -> None:
        # Completar
        tomar_bandera = self.lock_bandera.acquire(False)
        if tomar_bandera:
            if self.bandera.nombre_dueño == None:
                llamada = self.capturar_bandera()
    ...
    def intentar_robar_bandera(self) -> bool:
        ...
        if random() < self.PROBABILIDAD_ROBAR:
            llamada = self.intentar_capturar_bandera()
            if self.tiene_bandera:
    ...
    def correr_segunda_mitad(self) -> bool:
        ...
        with self.lock_carrera:
            if self.senal_fin.is_set():
                ...
                self._correr = False
                return True
            elif not self.tiene_bandera:
                llamada = self.intentar_robar_bandera()
        self.avanzar()
        time.sleep(Jugador.TIEMPO_ESPERA)
